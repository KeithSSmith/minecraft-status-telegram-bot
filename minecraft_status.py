...
class MinecraftStatus:
    ...
    def __init__(self, host='localhost', port=25565, timeout=5):
        ...
        self.current_player_count = None    # current number of players online
        self.current_player_list = []       # current list of player names online
    ...
    def get_status(self):
        ...
        try:
            ...
            self.motd = response['description']['text']
            self.current_player_count = response['players']['online']
            self.max_players = response['players']['max']
            self.latency = response['ping']
            if self.current_player_count > 0:
                temp_players_list = []
                for player in response['players']['sample']:
                    temp_players_list.append(player['name'])
                self.current_player_list = temp_players_list
            else:
